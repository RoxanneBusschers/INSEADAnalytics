...
setwd("C:/Users/Roxanne Busschers/Documents/R/BDA/CourseMaterial/INSEADAnalytics/Exercises/Exerciseset1/")
...
1. In the first lines of the script dataSet1, the source is set to be google and a vector of companies for which data needs to be loaded is set by 'mytickers' to included SPY and AAPL (mytickers = c("SPY", "AAPL")). For example facebook can be added by adding "FB" to the vector. 

2.Apple:
  Cumulative returns: 596.6%
  Average daily returns: 0.148%
  Standard deviation of daily returns: 2.389%

3.I added YHOO to the vector and found for Yahoo:
  Cumulative returns: 290.8%
  Average daily returns: 0.072%
  Standard deviation of daily returns: 3.019%  
...
1.  ncol(transposedData)
    nrow(transposedData)

2.  PortfolioTransp = apply(transposedData,2,mean)
    names(PortfolioTransp)<-colnames(transposedData)
    pnl_plot(PortfolioTransp)
    
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
PortfolioTransp = apply(transposedData,2,mean)
names(PortfolioTransp)<-colnames(transposedData)
pnl_plot(PortfolioTransp)
```
...
1. You can set the variable StartDate in the data.R file equal to '2001-01-01'. Then save this file and rerun the code.

2. Let's make a portfolio consisting of Apple, Facebook and Yahoo and display daily returns from 2002-01-01 till 2012-01-01. We sett the code in dataSet1.R to:
...
We change the definition of tmp to only take the dates uptill the defined enddate
...
Be aware: shown graph is not the one we wanted, because I had to change the code in dataSet1.R back to original.
...
write.csv(StockReturns[1:20,c("SPY","AAPL")], file = "twentydays.csv", row.names = TRUE) 
```
...
1. The data was not correctly stored in different columns, instead there is only 1 column with a string. I changed the seperater therefore to ",".
The answer is 60, so the data that is saved in myData is not equal to the original data in StockReturns. This seems to be due to a rounding issue.

2. I get an error, because StockReturns only has two columns (the first one contains both the dae and the returns) and myData has 3 columns (first with a date and second two with the returns). You can't add a string to a number.We can for example correct this with 
myData[,2]+StockReturns[1:40,]. But it would be better to make sure the data is correctly stored and retrieved.
...
We read some data that contains the number of people in the Netherlands in different age groups in different years.

```{r eval = TRUE, echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
myNLData <- read.csv(file = "C:/Users/Roxanne Busschers/Documents/INSEAD/P3/Big Data/Exercise set 1/Population_NL_age.csv", header = TRUE)

sum(myNLData[myNLData$Perioden==1950,2:6])
sum(myNLData[myNLData$Perioden==1980,2:6])
sum(myNLData[myNLData$Perioden==2016,2:6])

which.max(myNLData[myNLData$Perioden==1950,2:6])
which.max(myNLData[myNLData$Perioden==1980,2:6])
which.max(myNLData[myNLData$Perioden==2016,2:6])
```

We find the total number of inhabitants in 1950 to be 10,026,773, 1980: 140,091,014 and in 2016: 16,979,120 
The age groups are per 20 years, the biggest group was in 1950: janger than 20, in 1980: 20 to 40 and in 2016: 40 to 65 years.
