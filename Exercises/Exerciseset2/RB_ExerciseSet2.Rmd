...
getwd()
setwd("C:/Users/Roxanne Busschers/Documents/R/BDA/CourseMaterial/INSEADAnalytics/Exercises/Exerciseset2/")
list.files()
```
...
1. From the table it follows that you have to take 6 components to get above 50% of explained variance.
2. The weights of the first two factors, namely 5Y T-notes and 10Y T-notes are both 0.93. In total there are 10 factors high enough, with the values 

```{r,echo=FALSE}
as.data.frame(show_data[1:10,1],row.names=rownames(show_data[1:10,]))
```

3. 
      1. Bonds:       bonds of different duration and type
      2. Currencies
      3. EU_stock:    EU stock market indices
      4. US_stock:    US and Canada stock market indices
      5. EU_exch :    euro exchange rates and euro borrowing rates 
      6. OIL:         oil prices of different types of oil
      7. AL:          prices of commodities such as aluminium
      8. Corn:        prices of commodities such as soybeans and corn
      9. GOLD:        prices of gold, silver, platinum and palladium
      10. ASIAN:      Asian market (with Nikkei225 and Hang Seng bank of Hong Kong)
...
# display the data for the crisis and pre-crisis data
```{r}
...
Precrisis_eigenvalues  <- PreCrisis_Variance_Explained_Table[, "Eigenvalue"]
Precrisis_df           <- cbind(as.data.frame(Precrisis_eigenvalues), c(1:length(Precrisis_eigenvalues)), rep(1, length(Precrisis_eigenvalues)))
colnames(Precrisis_df) <- c("eigenvalues", "components", "abline")
iplot.df(melt(Precrisis_df, id="components"))
```


5. During the crisis the world was much more predictable: only 4 components can explain 50% of the variance. Before the crisis variables moved less together and we need 6 components to explain at least 50%. This makes sense, because during a crisis a bad financial environment exists anbd all prices go down.
...
1. In a for-loop the code determines in two steps the weights for each future for each day (the rows in our data):
  - first signal_used is updated to previous signal_used + sign. Where the sign is a signal that is negative or positive. 
  - then all weights of the futures are scaled to sum up to 1 (with the function r)

Then, the strategy is determined by taking the weights of two days earlier (since that is probably the most recent data you have at the moment the investment decision for the next day has to be made). Finally, the result of the strategy is determined by multiplying by the weights of the strategy by the returns of the futures.

2. MAfreq gives how often the trading strategy is updated. If we use a higher frequency (for example updating from 20 to 10, which doubles the number of moving averages), the strategy becomes less sensitive to small changes.
...
rmarkdown::render("C:/Users/Roxanne Busschers/Documents/R/BDA/CourseMaterial/INSEADAnalytics/Exercises/Exerciseset2/RB_ExerciseSet2.Rmd")
```
